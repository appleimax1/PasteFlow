...
enum DiffSegmentType {
    ...
    case inserted
    case deleted
    case modified(original: String, new: String, alternatives: [String])
}
...
// MARK: - Cached regular expressions (created once, reused forever)
private enum RegexCache {
    static let wordRegex = try! NSRegularExpression(
        pattern: "[А-Яа-яЁёA-Za-z]+(?:-[А-Яа-яЁёA-Za-z]+)*",
        options: []
    )
    static let spaceBeforePunct = try! NSRegularExpression(
        pattern: "\\s+([.,!?:;])",
        options: []
    )
    ...
    static let dashToEmDash = try! NSRegularExpression(
        pattern: "(\\s)-(\\s)",
        options: []
    )
    static let sentenceStart = try! NSRegularExpression(
        pattern: "([.!?]\\s+)([а-я])",
        options: []
    )
    static let quotes = try! NSRegularExpression(
        pattern: "\"([^\"]+)\"",
        options: []
    )
    static let kakRule = try! NSRegularExpression(
        pattern: "(?i)(?<=\\S)\\s+(как)\\s+([а-яёa-z0-9]+)",
        options: []
    )
    
    static let phraseReplacements: [(regex: NSRegularExpression, replacement: String)] = {
...
// MARK: - Ё-фиксация: частые пары е→ё (мгновенный lookup без Hunspell)
private let yoReplacementMap: [String: String] = [
    "еще": "ещё",
    "желтый": "жёлтый", "желтая": "жёлтая", "желтое": "жёлтое", "желтые": "жёлтые",
    "желто": "жёлто", "черный": "чёрный", "черная": "чёрная", "черное": "чёрное",
    "черные": "чёрные", "черно": "чёрно",
    "свекла": "свёкла",
    "оперенье": "оперёнье", "учет": "учёт", "учета": "учёта", "учету": "учёту",
    "учетом": "учётом", "учете": "учёте",
    "жесткий": "жёсткий", "жесткая": "жёсткая", "жесткое": "жёсткое",
    "жесткие": "жёсткие", "жестко": "жёстко",
    "дешевый": "дешёвый", "дешевая": "дешёвая", "дешевое": "дешёвое",
    "дешевые": "дешёвые", "дешево": "дешёво",
    "лен": "лён",
    "пестрый": "пёстрый", "пестрая": "пёстрая", "пестрое": "пёстрое",
    "пестрые": "пёстрые",
    "ржет": "ржёт",
    "щелк": "щёлк", "щелкать": "щёлкать", "щелчок": "щёлчок",
    "печен": "печён",
    "пленен": "пленён",
    "разорен": "разорён",
    "ребенок": "ребёнок",
...
class TextAssistantEngine {
    ...
    /// Hunspell engines keyed by language code.
    private lazy var hunspellEngines: [String: HunspellWrapper] = {
        var engines: [String: HunspellWrapper] = [:]
    ...
    /// Symspell index — disabled (caused quality issues)
    /// Kept for potential future use with better tuning
    private var symspellIndex: SymspellIndex? = nil

    /// Основной метод проверки текста (100% локальный, Hunspell + Ё-фиксация + Пунктуация)
    ...
    private func fixCommonSplitAndHyphenErrors(_ input: String) -> (text: String, fixCount: Int) {
        ...
        for p in RegexCache.phraseReplacements {
            let regex = p.regex
    ...
    private func fixRussianSpelling(_ input: String) -> (text: String, fixCount: Int) {
        ...
        for item in words.reversed() {
            if let replacement = processWord(item) {
    ...
    /// Select appropriate Hunspell engine for checking a word.
    private func engineForChecking(_ word: String) -> HunspellWrapper? {
        guard let firstChar = word.first else { return hunspellEngines["ru_RU"] ?? hunspellEngines.values.first }
        let isCyrillic = firstChar.unicodeScalars.contains { scalar in
            (0x0400...0x04FF).contains(scalar.value) || scalar.value == 0x0401 || scalar.value == 0x0451
        }
        if isCyrillic {
            return hunspellEngines["ru_RU"]
        } else {
            return hunspellEngines["en_US"]
        }
    }

    /// Process a single word. Returns replacement string if correction needed, nil otherwise.
    private func processWord(_ item: WordItem) -> String? {
        let word = item.word

        // Skip single-char words, numbers, ALL-CAPS acronyms
        if word.count < 2 || word.contains(where: { $0.isNumber }) || isAllCapsAcronym(word) {
            return nil
        }

        guard let engine = engineForChecking(word) else { return nil }

        // 0. Common overrides — always check first (handles slang regardless of Hunspell)
        let lower = word.lowercased()
        if let override = commonOverrideMap[lower] {
            return matchCase(original: word, suggestion: override)
        }

        // 1. Check correctness in original and lowercase
        let isWordCorrect = engine.isCorrect(word) || engine.isCorrect(word.lowercased())

        // 2. Ё-hypothesis: check precompiled yo-map first (instant lookup)
        if !isWordCorrect {
            if let yoReplacement = yoReplacementMap[lower] {
                return matchCase(original: word, suggestion: yoReplacement)
            }
        }

        // 3. Legacy Ё-hypothesis via Hunspell (for words not in map)
        if !isWordCorrect && word.contains("е") {
            let yoWord = word.replacingOccurrences(of: "е", with: "ё")
            if engine.isCorrect(yoWord) {
                return yoWord
            }
        }
        ...
        if !isWordCorrect {
            ...
            // Fallback: try lowercase suggestions for capitalized words
            if suggestions.isEmpty && word.first?.isUppercase == true {
                suggestions.append(contentsOf: engine.suggest(word.lowercased()))
            }

            if let best = selectBestCandidate(for: word, from: suggestions),
               !best.isEmpty,
               best.lowercased() != word.lowercased() {
                ...
                if dist <= maxAllowedDist {
                    return matchCase(original: word, suggestion: best)
                }
            }
        }
    ...
    /// Common overrides map — extracted to file-level for reuse
    private var commonOverrideMap: [String: String] {
        [
            // Сленг и разговорные
            "щас": "сейчас",
            "ща": "сейчас",
            ...
            "расcчитать": "рассчитать",

            // Контекстные ошибки (глаголы 3 лица: -ит → -ет)
            "будит": "будет",
        ]
    }
    ...
    /// Алгоритм выбора наилучшего кандидата: Hunspell suggestions + Levenshtein
    private func selectBestCandidate(for word: String, from suggestions: [String]) -> String? {
        ...
        for cand in suggestions {
            ...
            var shapeBonus = 0
            if lowerWord.count == lowerCand.count { shapeBonus += 3 }
            if lowerWord.prefix(1) == lowerCand.prefix(1) { shapeBonus += 2 }
            if lowerWord.suffix(1) == lowerCand.suffix(1) && lowerWord.count > 2 { shapeBonus += 2 }
            if commonWords.contains(lowerCand) { shapeBonus += 5 }
    ...
    private func fixRussianPunctuation(_ input: String) -> (text: String, fixCount: Int) {
        ...
        let kakRule = RegexCache.kakRule
        let matches = kakRule.matches(in: text, options: [], range: NSRange(location: 0, length: text.utf16.count))
        ...
        for word in introductoryWords {
            let startPattern = "(?i)(?<=[.!?]\\s|^)(" + NSRegularExpression.escapedPattern(for: word) + ")\\s+(?=[а-яёa-z0-9])"
        ...
        let spaceMatches = RegexCache.spaceBeforePunct.matches(in: text, options: [], range: NSRange(location: 0, length: text.utf16.count))
        if !spaceMatches.isEmpty {
            let mutable = NSMutableString(string: text)
            for match in spaceMatches.reversed() {
        ...
        let missingSpaceMatches = RegexCache.missingSpaceAfterPunct.matches(in: text, options: [], range: NSRange(location: 0, length: text.utf16.count))
        if !missingSpaceMatches.isEmpty {
            let mutable = NSMutableString(string: text)
            for match in missingSpaceMatches.reversed() {
        ...
        let dashMatches = RegexCache.dashToEmDash.matches(in: text, options: [], range: NSRange(location: 0, length: text.utf16.count))
        if !dashMatches.isEmpty {
            let mutable = NSMutableString(string: text)
            for match in dashMatches.reversed() {
                let range = match.range(at: 0)
                if range.location != NSNotFound {
        ...
        let sentenceMatches = RegexCache.sentenceStart.matches(in: text, options: [], range: NSRange(location: 0, length: text.utf16.count))
        if !sentenceMatches.isEmpty {
            let mutable = NSMutableString(string: text)
            for match in sentenceMatches.reversed() {
                let range = match.range(at: 2)
                if range.location != NSNotFound {
        ...
        let quoteMatches = RegexCache.quotes.matches(in: text, options: [], range: NSRange(location: 0, length: text.utf16.count))
        ...
        // Правило 9: Автоматическая точка в конце предложения
    ...
    private func extractWordsWithRanges(from text: String) -> [WordItem] {
        ...
        let regex = RegexCache.wordRegex
            ...
            while i < origWords.count || j < corrWords.count {
                if i < origWords.count && j < corrWords.count {
                    ...
                    if w1 == w2 {
                        ...
                        if let engine = self.engineForChecking(w1) {
                            let suggestions = engine.suggest(w1)
                            let filtered = suggestions.filter { $0.lowercased() != w1.lowercased() && $0.lowercased() != w2.lowercased() }
                            alternatives = Array(filtered.prefix(4))
                        }
                        segments.append(TextDiffSegment(text: w2, type: .modified(original: w1, new: w2, alternatives: alternatives)))
                        i += 1
                        j += 1
                    }
